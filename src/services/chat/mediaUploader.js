...
export function uploadMediaFile(file, onProgress, meta = {}) {
  ...
  try { console.info('[UPLOAD] form entries', entries); } catch (e) {}
      ...
      try {
        // Use authClient for consistent token retrieval
        let token = null;
        try {
          if (typeof window !== 'undefined') {
            const authClient = require('../authClient').default;
            token = authClient.getAccessToken();
          }
        } catch (e) {
          // Fallback to localStorage if authClient not available
          const raw = typeof window !== 'undefined' ? localStorage.getItem('token') : null;
          if (raw) {
            token = String(raw).replace(/^Token\s*/i, '').replace(/^Bearer\s*/i, '');
          }
        }
        if (token) {
          ...
          try { console.info('[UPLOAD] no token found'); } catch(e){}
        }
      } catch (e) {
