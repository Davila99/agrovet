...
export default function ResetByPhone() {
  ...
  async function handleRequest(e) {
    ...
    setMsg("Código enviado. Revisa el SMS (o la consola en dev). Si no llega, espera unos minutos.");
    setStep(2);
  }
  ...
  return (
    <Box sx={{ maxWidth: 480, mx: "auto", p: 2 }}>
      ...
      {step === 1 && (
        <Box component="form" onSubmit={handleRequest} sx={{ display: "grid", gap: 2 }}>
          <TextField label="Teléfono (ej: +505 9XXXX-XXXX)" value={phone} onChange={(e) => setPhone(e.target.value)} />
      ...
      {step === 2 && (
        <Box component="form" onSubmit={handleVerify} sx={{ display: "grid", gap: 2 }}>
          <TextField label="Código (6 dígitos)" value={code} onChange={(e) => setCode(e.target.value)} />
          <TextField label="Nueva contraseña" type="password" value={newPass} onChange={(e) => setNewPass(e.target.value)} />
          <Box sx={{ display: "flex", gap: 1 }}>
            <Button variant="contained" type="submit">Verificar y cambiar</Button>
