  ...
  return (
    <Box sx={{ 
      ...
      <svg 
        width="100%" 
        height={waveHeight} 
        viewBox={`0 0 ${bins * (barWidth + barGap)} ${waveHeight}`} 
      ...
      >
        {scaled.map((v, i) => {
          ...
          // Líneas verticales en lugar de rectángulos
          return (
            <line 
              key={i} 
              x1={x} 
              ...
              strokeLinecap="round"
              opacity={opacity} 
              style={{ transition: 'stroke 140ms linear, opacity 140ms linear' }} 
            />
          );
        })}
      </svg>
      ...
      <Box sx={{ 
        position: 'absolute', 
        left: `calc(${Math.max(0, Math.min(1, progress || 0)) * 100}% - 6px)`, 
        top: (waveHeight / 2) - 6, 
        width: 12, 
        height: 12, 
        borderRadius: '50%', 
