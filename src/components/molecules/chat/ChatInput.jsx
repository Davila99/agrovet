  ...
  return (
    ...
    >
      ...
      <TextField
        ...
        sx={{ '& .MuiOutlinedInput-root': { borderRadius: { xs: 20, sm: 24, md: 28 }, paddingRight: 0, backgroundColor: (theme) => recording ? '#f5f5f5' : (theme.palette.mode === 'light' ? '#fff' : '#111') } }}
        InputProps={{
          inputRef: inputRef,
          startAdornment: (
              <InputAdornment position="start">
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, pl: 0.5 }}>
                  ...
                  {((pendingAttachment || localPending) && (pendingAttachment || localPending).file && typeof (pendingAttachment || localPending).file.type === 'string' && (pendingAttachment || localPending).file.type.startsWith('audio') && Array.isArray((pendingAttachment || localPending).spectrum)) && (
                    <Box sx={{ ml: 1, display: 'flex', alignItems: 'flex-end', gap: '1px', width: { xs: 90, sm: 140 }, height: 28 }}>
            ...
            endAdornment: (
              <InputAdornment position="end">
                ...
                {(String(text || '').trim().length > 0 || hasImageAttachment) ? (
                  <SendButton onClick={handleSend} disabled={!!sending || !!uploadingAttachment} />
                ) : (
                          ...
                          <Tooltip title="Enviar">
                            <IconButton size="small" onClick={() => {
                              try {
                                ...
                                const attachmentToSend = pendingAttachment || localPending;
      ...
      <Box sx={{ display: 'none' }}>
        <AudioRecorder 
          ref={recorderRef} 
          onAttach={(payload) => { try { if (typeof onAttach === 'function') onAttach(payload); setLocalPending(payload); } catch(e){} }} 
          onCancelAttachment={(ev) => { try { if (typeof onCancelAttachment === 'function') onCancelAttachment(ev); setLocalPending(null); } catch(e){} }} 
          onConfirmAttachment={(ev) => { try { if (typeof onConfirmAttachment === 'function') onConfirmAttachment(ev); setLocalPending(null); } catch(e){} }} 
