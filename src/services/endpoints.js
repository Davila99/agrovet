...
// Helper to normalize token strings
function normToken(t) {
  if (!t) return null;
  return String(t).replace(/^Token\s*/i, '').replace(/^Bearer\s*/i, '');
}

// Simple wrappers that use normalized Token auth header
export const authHeaders = (token) => {
  const raw = normToken(token);
  return raw ? { Authorization: `Token ${raw}` } : {};
};

// Chat endpoints
export const chatAPI = {
  // Create a room (participants: array of user ids)
  // The backend serializer expects `participants_ids` as the write key
  // (it maps to `participants` server-side). Send that key here so DRF
  // accepts the payload without a 400.
  createRoom: (participants = [], is_private = false) =>
    ({ token } = {}) => httpClient(`/chat/rooms/`, {
      method: "POST",
      headers: authHeaders(token),
      body: { participants_ids: participants, is_private },
    }),
  // List rooms for current user (backend should implement filtering)
  listRooms: ({ token } = {}) => httpClient(`/chat/rooms/`, { method: "GET", headers: authHeaders(token) }),
  // Send message via REST (fallback when WebSocket isn't open)
  sendMessage: (roomId, content) =>
    ({ token } = {}) => httpClient(`/chat/messages/`, { method: "POST", headers: authHeaders(token), body: { room: roomId, content } }),
  // Fetch last messages for a room (server action)
  getLastMessages: (roomId, limit = 50) =>
    ({ token } = {}) => httpClient(`/chat/messages/last_messages/?room=${encodeURIComponent(roomId)}&limit=${encodeURIComponent(limit)}`, { method: "GET", headers: authHeaders(token) }),
};
